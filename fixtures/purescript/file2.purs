...
-- Get students at risk (average < 70)
atRiskStudents :: Array Student -> Array Student
atRiskStudents students =
  filter (\s -> average s.grades < 70.0) students

main :: Effect Unit
main = do
  let students =
        [ { id: 1, name: "Alice", grades: [95.0, 88.0, 92.0], major: "CS" }
        , { id: 2, name: "Bob",   grades: [62.0, 58.0, 65.0], major: "Math" }
        , { id: 3, name: "Carol", grades: [91.0, 95.0, 89.0], major: "CS" }
        ]
  let atRisk = atRiskStudents students
  logShow $ map _.name atRisk
